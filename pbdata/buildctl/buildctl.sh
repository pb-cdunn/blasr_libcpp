...
install_build() {
    ...
    for i in . alignment matrix metagenome qvs reads saf sam utils; do
	mkdir -p "$g_installbuild_dir/include/$i"
	cp -a "${g_progdir}/../$i"/*.hpp "$g_installbuild_dir/include/$i"
    done
    for i in . sam; do
	mkdir -p "$g_installbuild_dir/include/$i"
	cp -a "${g_progdir}/../$i"/*.h "$g_installbuild_dir/include/$i"
    done
